...
public class Collectable_UI : MonoBehaviour
{
	public Camera cam;
	...
	void OnTriggerEnter(Collider col)
	{
		if (col.tag == "Player")
		{
			...
			collectible_type = GetComponent<CollectibleType>().CollectType;

			//Get the point on the screen that relates to the point in the world that we picked this up
			WorldToScreen = cam.WorldToScreenPoint(transform.position);
