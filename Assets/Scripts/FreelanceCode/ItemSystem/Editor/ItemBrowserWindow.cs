...
{
	public class ItemBrowserWindow: FoCsWindow<ItemBrowserWindow>
	{
		...
		private static void Init()
		{
			...
			Window.minSize = new Vector2(750, 230);
		}
		...
		private const float  ASSET_PANEL_PERCENT   = 0.35f;
		...
		protected override void OnGUI()
		{
			using(FoCsEditor.Disposables.HorizontalScope())
			{
				using(FoCsEditor.Disposables.VerticalScope(GUILayout.Width(Screen.width * ASSET_PANEL_PERCENT), GUILayout.MinWidth(ASSET_PANEL_MIN_WIDTH)))
				{
					...
					using(var scroll = FoCsEditor.Disposables.ScrollViewScope(scrollPos, true, GUILayout.Width(Screen.width * ASSET_PANEL_PERCENT), GUILayout.MinWidth(ASSET_PANEL_MIN_WIDTH)))
					{
		...
		static readonly GUIContent Ping = new GUIContent("","Ping Item");
		private bool DrawItemLabel(int i)
		{
			...
			using(FoCsEditor.Disposables.HorizontalScope())
			{
				if(FoCsGUI.Layout.Button(Ping, FoCsGUI.Styles.Find, GUILayout.Width(16)))
				{
					EditorGUIUtility.PingObject(ItemAssets[i]);
				}
				...
				{
					var @event = FoCsGUI.Layout.Toggle(ItemAssets[i].name.SplitCamelCase(), ActiveIndex == i, (i % 2) == 0? FoCsGUI.Styles.RowField : FoCsGUI.Styles.RowOddField, ToggleOp);
		...
		public static void DrawCreateNewItem(AdvPopupWindowArguments args)
		{
			...
			FoCsGUI.Layout.Label("Assets will be saved to \"Assets/Resources/ItemSystem/Items\"");

			var path    = string.Format("Assets/Resources/ItemSystem/Items/{0}.asset", newItem.name);
		...
		private static void DoSaveAssetToDisk(string path, bool hasFile)
		{
			...
			if(!hasFile)
			{
				if(EditorUtility.DisplayDialog(string.Format("Overwrite Asset:{0}", path), string.Format("Are you sure you want to overwrite the asset at path:{0}", path), "Yes, overwrite.", "No, Cancel."))
				{
					...
					AssetDatabase.CreateAsset(newItem, string.Format("Assets/Resources/ItemSystem/Items/{0}.asset", newItem.name));
				}
			}
			else
			{
		...
		private static void CreateAssetsFolder()
		{
			...
			if(!AssetDatabase.IsValidFolder("Assets/Resources/ItemSystem"))
				AssetDatabase.CreateFolder("Assets/Resources", "ItemSystem");

			if(!AssetDatabase.IsValidFolder("Assets/Resources/ItemSystem/Items"))
				AssetDatabase.CreateFolder("Assets/Resources/ItemSystem", "Items");
		}
